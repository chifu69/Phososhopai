const CACHE='photo-ia-15-40-0-advanced-selection';
const VERSION='15.40.0';
const CORE=[
 './','./index.html','./styles.css?v=15.40.0','./ui-layout.js?v=15.40.0','./app.js?v=15.40.0',
 './creative-tools.js?v=15.40.0','./curves-tool.js?v=15.40.0','./mask-tool.js?v=15.40.0','./brain.js?v=15.40.0',
 './opencv-engine.js?v=15.40.0','./smart-core.js?v=15.40.0','./wardrobe-engine.js?v=15.40.0','./ai-studio.js?v=15.40.0',
 './vision.js?v=15.40.0','./onnx-engine.js?v=15.40.0','./content-aware-ai.js?v=15.40.0','./segmentation.js?v=15.40.0','./smart-select.js?v=15.40.0','./segmentation-worker.js?v=15.40.0','./body-retouch.js?v=15.40.0','./manifest.webmanifest','./assets/icons/icon-192.png','./assets/icons/icon-512.png'
];
const ASSETS=[
 ...
 {local:'./assets/vendor/fabric.min.js?v=15.40.0',remote:'https://cdnjs.cloudflare.com/ajax/libs/fabric.js/5.3.1/fabric.min.js',name:'Editor de capas',required:true},
 {local:'./assets/vendor/cropper.min.js?v=15.40.0',remote:'https://cdnjs.cloudflare.com/ajax/libs/cropperjs/1.6.2/cropper.min.js',name:'Herramienta de recorte',required:true},
 {local:'./assets/vendor/cropper.min.css?v=15.40.0',remote:'https://cdnjs.cloudflare.com/ajax/libs/cropperjs/1.6.2/cropper.min.css',name:'Estilos de recorte',required:true},
 {local:'./assets/vendor/tf.min.js?v=15.40.0',remote:'https://cdn.jsdelivr.net/npm/@tensorflow/tfjs@4.22.0/dist/tf.min.js',name:'TensorFlow móvil',required:false},
 {local:'./assets/vendor/coco-ssd.min.js?v=15.40.0',remote:'https://cdn.jsdelivr.net/npm/@tensorflow-models/coco-ssd@2.2.3/dist/coco-ssd.min.js',name:'Detector de objetos',required:false},
 {local:'./assets/vendor/opencv.js?v=15.40.0',remote:'https://docs.opencv.org/4.x/opencv.js',name:'OpenCV avanzado',required:false},
 {local:'./assets/mediapipe/vision_bundle.mjs?v=15.40.0',remote:'https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision@1.0.1/vision_bundle.mjs',name:'MediaPipe Vision ESM 1.0.1',required:false,timeout:30000},
 ...
 {local:'./assets/vendor/ort.min.js?v=15.40.0',remote:'https://cdn.jsdelivr.net/npm/onnxruntime-web@1.23.0/dist/ort.min.js',name:'ONNX Runtime Web',required:false,timeout:30000},
...
async function cacheRemote(asset){
 ...
 // Keep both aliases: PHOTO IA's same-origin virtual path and the original
 // CDN URL used by ORT dynamic module imports on Safari/iOS.
 await cache.put(localKey(asset.local),response.clone());
 try{await cache.put(asset.remote,response.clone())}catch(_){ }
}
...
self.addEventListener('activate',e=>e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith('photo-ia-')&&k!==CACHE).map(k=>caches.delete(k)))).then(()=>self.clients.claim())));
...
self.addEventListener('fetch',e=>{
 ...
 const remoteAsset=ASSETS.find(a=>a.remote===e.request.url);
 if(remoteAsset){
  e.respondWith((async()=>{
   const cache=await caches.open(CACHE);
   ...
   if(hit)return hit;
   try{
    const fresh=await fetchWithTimeout(remoteAsset.remote,remoteAsset.timeout||(remoteAsset.required?25000:12000));
    if(fresh.ok){try{await cache.put(e.request,fresh.clone())}catch(_){ }return fresh}
    return fresh;
   }catch(err){return new Response('Offline resource unavailable',{status:503,headers:{'Content-Type':'text/plain'}})}
  })());
  return;
 }
 ...
 e.respondWith((async()=>{
  ...
  if(asset){
   ...
   catch(err){return new Response('Asset unavailable',{status:503,headers:{'Content-Type':'text/plain'}})}
  }
