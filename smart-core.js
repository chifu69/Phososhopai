(() => {
'use strict';
const VERSION='8.5.0-adaptive-local-core';
...
function boot(){renderQueue();$('smart-analyze')?.addEventListener('click',()=>analyze().catch(e=>{console.error(e);setStatus('No se pudo analizar','error');api()?.toast(e.message)}));$('smart-apply')?.addEventListener('click',()=>applyRecommendations().catch(e=>{console.error(e);setStatus('Error','error');api()?.toast(e.message)}));document.querySelectorAll('[data-smart-mode]').forEach(b=>b.addEventListener('click',()=>applyMode(b.dataset.smartMode).catch(e=>api()?.toast(e.message))));$('smart-queue-clear')?.addEventListener('click',()=>saveQueue([]));const input=$('command-input');input?.addEventListener('input',()=>{const text=input.value.trim();$('smart-route-hint').textContent=text?explainRoute(text):'PHOTO IA decidirá si la tarea se hace aquí o se guarda para la PC.';});registerBrain();window.PhotoSmartCore={version:VERSION,analyze,applyRecommendations,applyMode,classify,queueForPC,loadQueue,capabilities};setStatus('Núcleo adaptativo listo','ready');}
