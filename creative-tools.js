...
const onReady=()=>{
  ...
  function setCanvasMode(mode,{openPanel=true,announce=true}={}){
    ...
    canvas.defaultCursor=mode==='erase'?'not-allowed':(mode==='draw'||mode==='mask')?'crosshair':'default';
    ...
    if(mode!=='mask')canvas.discardActiveObject();
    ...
    if(openPanel){
      const tabMap={draw:'draw',sticker:'stickers',text:'text',shape:'shapes',mask:'mask'};
